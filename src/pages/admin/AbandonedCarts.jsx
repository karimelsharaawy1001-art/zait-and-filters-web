...
const AbandonedCarts = () => {
    ...
    const getStageBadge = (stage) => {
        const styles = {
            'Cart Page': 'bg-[#ffffff05] text-admin-text-secondary border-[#ffffff0d]',
            'Shipping Info': 'bg-admin-yellow/5 text-admin-yellow border-admin-yellow/10',
            'Payment Selection': 'bg-purple-400/5 text-purple-400 border-purple-400/10'
        };
        return (
            <span className={`px-2 py-0.5 rounded border text-[9px] font-black uppercase tracking-widest ${styles[stage] || 'bg-[#ffffff05] text-admin-text-secondary border-[#ffffff0d]'}`}>
                {stage || 'Unknown'}
            </span>
        );
    };
    ...
    return (
        <div className="p-6 space-y-6 bg-admin-bg min-h-screen">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h1 className="text-2xl font-black text-white flex items-center gap-3 uppercase tracking-widest poppins">
                ...
                <div className="flex items-center gap-2 bg-[#ffffff05] p-1.5 rounded-2xl border border-admin-border shadow-lg">
                    {['all', 'pending', 'recovered'].map((f) => (
                        <button
                            key={f}
                            onClick={() => setFilter(f)}
                            className={`px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${filter === f ? 'bg-admin-red hover:bg-admin-red-dark text-white shadow-lg shadow-admin-red/40' : 'text-gray-500 hover:text-white'}`}
                        >
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-20 gap-4">
                    ...
                    <p className="text-white font-black uppercase tracking-widest text-[10px]">Loading carts...</p>
                </div>
            ) : filteredCarts.length === 0 ? (
                <div className="bg-admin-card rounded-3xl border border-dashed border-[#ffffff0d] p-20 text-center shadow-admin">
                    <ShoppingBag className="w-16 h-16 text-gray-700 mx-auto mb-4" />
                    <h3 className="text-lg font-black text-white uppercase tracking-widest poppins">No abandoned carts found</h3>
                ...
                <div className="bg-admin-card rounded-3xl shadow-admin border border-admin-border overflow-hidden">
                    <div className="overflow-x-auto">
                        <table className="w-full text-left">
                            <thead className="bg-[#ffffff02] border-b border-[#ffffff0d]">
                                <tr>
                            ...
                            <tbody className="divide-y divide-[#ffffff0d]">
                                {filteredCarts.map((cart) => (
                                    <tr key={cart.id} className="hover:bg-[#ffffff02] transition-colors group">
                                        <td className="px-8 py-5">
                                            <div className="flex flex-col gap-1.5">
                                                <span className="text-sm font-black text-white flex items-center gap-2 poppins">
                                        ...
                                        <td className="px-8 py-5">
                                            <div className="flex items-center justify-end gap-2 translate-x-2 opacity-0 group-hover:opacity-100 group-hover:translate-x-0 transition-all">
                                                <button
                                                    ...
                                                    className="p-2.5 text-gray-500 hover:text-white hover:bg-[#ffffff0d] rounded-xl transition-all shadow-lg"
                                                    title="View Details"
                                                >
            ...
            {isModalOpen && selectedCart && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
                    <div className="bg-admin-card rounded-[2rem] shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-hidden flex flex-col animate-in fade-in zoom-in duration-200 border border-admin-border">
                        <div className="p-8 border-b border-[#ffffff0d] flex items-center justify-between bg-[#ffffff02]">
                            <div>
                                <h2 className="text-xl font-black text-white uppercase tracking-widest poppins">Cart Details</h2>
                                <p className="text-[10px] text-gray-500 font-black mt-1 uppercase tracking-widest">ID: {selectedCart.id}</p>
                            </div>
                            <button
                                onClick={() => setIsModalOpen(false)}
                                className="p-3 bg-[#ffffff05] hover:bg-[#ffffff0d] rounded-2xl transition-all text-gray-500 hover:text-white border border-admin-border shadow-lg"
                            >
                                <X className="w-6 h-6" />
                            </button>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-8 space-y-8">
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="p-5 rounded-2xl bg-[#ffffff03] border border-admin-border shadow-inner">
                                    <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2 block">Customer</label>
                                    <p className="text-sm font-black text-white poppins">{selectedCart.customerName || 'Anonymous Guest'}</p>
                                </div>
                                <div className="p-5 rounded-2xl bg-[#ffffff03] border border-admin-border shadow-inner">
                                ...
                                <div className="p-5 rounded-2xl bg-[#ffffff03] border border-admin-border shadow-inner col-span-2">
                                    <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2 block">Email</label>
                                    <p className="text-sm font-black text-purple-400 poppins">{selectedCart.email || 'N/A'}</p>
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-3">
                                    {selectedCart.items?.map((item, idx) => (
                                        <div key={idx} className="flex gap-5 p-4 rounded-2xl bg-[#ffffff02] border border-admin-border hover:border-admin-accent/30 transition-all items-center shadow-lg">
                                            <div className="w-20 h-20 rounded-2xl overflow-hidden bg-[#ffffff05] flex-shrink-0 border border-admin-border shadow-md">
                                                <img src={item.image} alt={item.name} className="w-full h-full object-cover" />
                                            </div>
                                            <div className="flex-1 min-w-0">
                                                <p className="text-sm font-black text-white poppins mb-1">{item.name}</p>
                                                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">{item.brand} • {item.quantity} units</p>
                                            </div>
                                            <div className="text-right">
                                                <p className="text-lg font-black text-white poppins">{item.price?.toLocaleString()} <span className="text-[10px] opacity-40">EGP</span></p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            {/* Cart Summary */}
                            <div className="p-8 rounded-3xl bg-gradient-to-br from-admin-accent/10 to-transparent border border-admin-accent/20 shadow-lg shadow-admin-accent/5">
                                <div className="flex items-center justify-between mb-2">
                                    <span className="text-sm font-black text-admin-accent uppercase tracking-widest poppins">Potential Value</span>
                                    <span className="text-3xl font-black text-white poppins">{selectedCart.total?.toLocaleString()} <span className="text-sm font-normal text-gray-500">EGP</span></span>
                                </div>
                                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Calculated from item total values</p>
                            </div>
                        </div>

                        <div className="p-8 bg-[#ffffff02] border-t border-[#ffffff0d] flex gap-4">
                            <button
                                onClick={() => openWhatsApp(selectedCart.customerPhone, selectedCart)}
                                className="flex-1 bg-gradient-to-r from-admin-green to-[#2dcecc] text-white font-black py-4 rounded-xl flex items-center justify-center gap-3 transition-all shadow-lg shadow-admin-green/20 hover:scale-105 uppercase tracking-widest text-xs"
                            >
                                <MessageCircle className="w-5 h-5" />
                                WhatsApp Reminder
                            </button>
