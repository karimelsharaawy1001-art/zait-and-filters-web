          ...
          >
            ...
            <div style={{ padding: '20px 20px 16px', borderBottom: '1px solid #f3f4f6', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                ...
                <span style={{ fontWeight: '900', fontSize: '1.05rem', color: '#1a1a1a' }}>سلتك</span>
                {totalQty > 0 && (
                  <span style={{ background: '#f0fdf4', color: '#15803d', borderRadius: '20px', padding: '2px 10px', fontSize: '0.75rem', fontWeight: '800', border: '1px solid #166534' }}>
                    {totalQty} قطعة
                  </span>
                )}
              </div>
              <button onClick={() => setOpen(false)} style={{ width: '32px', height: '32px', borderRadius: '50%', border: 'none', background: '#f3f4f6', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280' }}>
                <X size={16} />
              </button>
            </div>
            ...
            <div style={{ flex: 1, overflowY: 'auto', padding: '16px' }}>
              {cart.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '60px 20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '12px' }}>
                  ...
                  <p style={{ fontWeight: '800', color: '#6b7280', margin: 0, fontSize: '0.95rem' }}>سلتك فارغة</p>
                  ...
                  {cart.map((item: any) => {
                    ...
                    return (
                      <m.div key={item.id}
                        ...
                        style={{ display: 'flex', gap: '12px', padding: '14px 0', borderBottom: '1px solid #f9fafb' }}>
                        {/* Image */}
                        <div style={{ width: '72px', height: '72px', borderRadius: '14px', overflow: 'hidden', background: '#f9fafb', border: '1px solid #f3f4f6', flexShrink: 0 }}>
                          {img ? <img src={optimizeImageUrl(img)} alt={item.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} /> : (
                            <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><ShoppingBag size={22} color="#374151" /></div>
                          )}
                        </div>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ display: 'flex', justifyContent: 'space-between', gap: '6px', marginBottom: '4px' }}>
                            ...
                            <button onClick={() => removeFromCart(item.id)}
                              style={{ border: 'none', background: 'none', cursor: 'pointer', color: '#374151', padding: '2px', display: 'flex', flexShrink: 0, transition: 'color 0.15s' }}
                              onMouseEnter={e => (e.currentTarget.style.color = '#16a34a')}
                              onMouseLeave={e => (e.currentTarget.style.color = '#374151')}>
                              <Trash2 size={13} />
                            </button>
                          </div>
                          <p style={{ margin: '0 0 4px', fontSize: '0.85rem', fontWeight: '800', color: '#1a1a1a', overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' as const }}>
                            {item.name}
                          </p>
                          {!isUniversal && compatText && (
                            <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px', fontSize: '0.65rem', color: '#15803d', background: '#f0fdf4', border: '1px solid #166534', borderRadius: '10px', padding: '1px 6px' }}>
                              <Car size={9} />{compatText}
                            </span>
                          )}
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '8px' }}>
                            <span style={{ fontSize: '0.95rem', fontWeight: '900', color: '#1a1a1a' }}>{(price * item.quantity).toFixed(2)} ج.م</span>
                            {/* Qty stepper */}
                            <div style={{ display: 'flex', alignItems: 'center', background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '10px', overflow: 'hidden' }}>
                              <button onClick={() => addToCart(item, 1)} style={{ width: '28px', height: '28px', border: 'none', background: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280' }}><Plus size={12} /></button>
                              <span style={{ width: '28px', textAlign: 'center', fontWeight: '900', fontSize: '0.85rem' }}>{item.quantity}</span>
                              <button onClick={() => item.quantity > 1 && removeFromCart(item.id, true)} style={{ width: '28px', height: '28px', border: 'none', background: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280' }}><Minus size={12} /></button>
                            </div>
                          </div>
                        </div>
                      </m.div>
            ...
            {/* Footer */}
            {cart.length > 0 && (
              <div style={{ padding: '16px 20px 24px', borderTop: '1px solid #f3f4f6', flexShrink: 0, background: '#ffffff' }}>
                {/* Total */}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
                  <span style={{ fontWeight: '700', color: '#6b7280', fontSize: '0.88rem' }}>الإجمالي</span>
                  <span style={{ fontSize: '1.3rem', fontWeight: '900', color: '#1a1a1a' }}>{getCartTotal().toFixed(2)} <small style={{ fontSize: '0.75rem' }}>ج.م</small></span>
                </div>
                {/* CTA */}
                ...
                <Link href="/cart" onClick={() => setOpen(false)}
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', width: '100%', padding: '11px', background: '#f9fafb', color: '#6b7280', textDecoration: 'none', borderRadius: '12px', fontWeight: '700', fontSize: '0.82rem', border: '1px solid #e5e7eb' }}>
                  عرض السلة كاملة
                </Link>
                {/* Trust */}
                <div style={{ display: 'flex', justifyContent: 'center', gap: '16px', marginTop: '12px', flexWrap: 'wrap' as const }}>
                  {['🔒 دفع آمن', '🚚 شحن سريع', '✅ قطع أصلية'].map((t, i) => (
                    <span key={i} style={{ fontSize: '0.68rem', color: '#6b7280', fontWeight: '700' }}>{t}</span>
                  ))}
                </div>
              </div>
            )}
