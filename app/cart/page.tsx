...
export default function CartPage() {
  ...
  if (!cart || cart.length === 0) return (
    <div style={{ textAlign: 'center', padding: '80px 20px', direction: 'rtl', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '16px' }}>
      <div style={{ width: '96px', height: '96px', borderRadius: '50%', background: '#f3f4f6', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <ShoppingBag size={44} color="#374151" />
      </div>
      <h2 style={{ fontSize: '1.4rem', fontWeight: '900', color: '#1a1a1a', margin: 0 }}>سلتك فارغة</h2>
      <p style={{ color: '#6b7280', margin: 0, fontSize: '0.95rem' }}>لم تضف أي منتجات بعد — ابدأ التسوق الآن!</p>
    ...
    <>
      <style>{`
        * { box-sizing: border-box; }
        ...
        .cart-item { display: flex; background: #ffffff; border-radius: 20px; border: 1px solid #f3f4f6; gap: 16px; padding: 16px; box-shadow: 0 2px 12px rgba(0,0,0,0.04); transition: box-shadow 0.2s; }
        .cart-item:hover { box-shadow: 0 6px 24px rgba(0,0,0,0.08); }
        .cart-item-img { width: 100px; height: 100px; border-radius: 16px; overflow: hidden; background: #f9fafb; border: 1px solid #f3f4f6; flex-shrink: 0; }
        .cart-item-img img { width: 100%; height: 100%; object-fit: cover; }
        .item-info { flex: 1; display: flex; flex-direction: column; gap: 6px; min-width: 0; }
        .item-name { font-size: 0.95rem; font-weight: 900; color: #1a1a1a; margin: 0; word-break: break-word; line-height: 1.4; }
        ...
        .item-compat { display: inline-flex; align-items: center; gap: 5px; font-size: 0.72rem; font-weight: 700; color: #15803d; background: #f0fdf4; border: 1px solid #166534; border-radius: 12px; padding: 2px 8px; align-self: flex-start; overflow: hidden; max-width: 100%; }
        ...
        .item-price { font-size: 1.1rem; font-weight: 900; color: #1a1a1a; }
        ...
        .summary-card { background: #ffffff; border-radius: 24px; border: 1px solid #f3f4f6; box-shadow: 0 4px 24px rgba(0,0,0,0.05); padding: 24px; }
        .sum-row { display: flex; justify-content: space-between; align-items: center; font-size: 0.88rem; color: #6b7280; margin-bottom: 10px; }
        .sum-total-row { display: flex; justify-content: space-between; align-items: center; padding-top: 14px; margin-top: 6px; border-top: 1.5px solid #f3f4f6; }
      ...
      <div className="cart-wrap">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
            ...
            <h1 style={{ margin: 0, fontSize: '1.4rem', fontWeight: '900', color: '#1a1a1a' }}>سلة المشتريات</h1>
            <span style={{ background: '#f0fdf4', color: '#15803d', borderRadius: '20px', padding: '3px 12px', fontSize: '0.8rem', fontWeight: '800', border: '1px solid #166534' }}>{totalQty} {totalQty === 1 ? 'قطعة' : 'قطع'}</span>
          </div>
          <Link href="/store" style={{ display: 'flex', alignItems: 'center', gap: '5px', color: '#6b7280', textDecoration: 'none', fontSize: '0.85rem', fontWeight: '700' }}>
        ...
        <div className="cart-grid">
                    ...
                    <div className="cart-item-img">
                      {img ? <img src={optimizeImageUrl(img)} alt={item.name} /> : (
                        <div style={{ width: '100%', height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <ShoppingBag size={28} color="#374151" />
                        </div>
                      )}
                    </div>
          ...
          <aside className="cart-summary">
            <div className="summary-card">
              <h2 style={{ margin: '0 0 16px', fontSize: '1.05rem', fontWeight: '900', color: '#1a1a1a' }}>ملخص الطلب</h2>

              <div className="sum-row">
                <span>المنتجات ({totalQty} قطعة)</span>
                <span style={{ fontWeight: '800', color: '#1a1a1a' }}>{getCartTotal().toFixed(2)} ج.م</span>
              </div>
              <div className="sum-row">
                <span>الشحن</span>
                <span style={{ fontWeight: '700', color: '#6b7280', fontSize: '0.82rem' }}>يُحدد عند الطلب</span>
              </div>

              <div className="sum-total-row">
                <span style={{ fontWeight: '900', color: '#1a1a1a', fontSize: '0.95rem' }}>الإجمالي</span>
                <span style={{ fontSize: '1.4rem', fontWeight: '900', color: '#1a1a1a' }}>{getCartTotal().toFixed(2)} <span style={{ fontSize: '0.85rem' }}>ج.م</span></span>
              </div>
