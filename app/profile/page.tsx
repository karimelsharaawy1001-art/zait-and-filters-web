...
const Select = dynamic(() => import('react-select'), {
  ssr: false,
  loading: () => <div style={{ height: '48px', backgroundColor: '#f9fafb', borderRadius: '10px', padding: '0 15px', display: 'flex', alignItems: 'center', color: '#6b7280' }}>جاري التحميل...</div>
});
...
const shippingStatusConfig: Record<string, { label: string; bg: string; color: string }> = {
  pending:    { label: 'قيد المراجعة',   bg: '#ffffff', color: '#fb923c' },
  processing: { label: 'جاري التجهيز',  bg: '#f3f4f6', color: '#a16207' },
  shipped:    { label: 'تم الشحن 🚚',   bg: '#f9fafb', color: '#60a5fa' },
  delivered:  { label: 'تم التسليم ✓',  bg: '#f0fdf4', color: '#15803d' },
  pending_payment: { label: 'في انتظار الدفع', bg: '#f9fafb', color: '#60a5fa' },
};
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        border: '1px solid #166534', display: 'flex', alignItems: 'center',
        justifyContent: 'space-between', flexWrap: 'wrap', gap: '10px',
      }}>
        <div>
          <div style={{ fontSize: '0.65rem', fontWeight: '700', color: '#9ca3af', marginBottom: '3px', letterSpacing: '0.5px' }}>
            رقم تتبع الشحنة
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <button
              ...
              style={{
                ...
                border: copied ? '1px solid #16a34a' : '1px solid #e5e7eb',
...
export default function ProfilePage() {
      ...
      <div style={{
        ...
        <div onClick={e => e.stopPropagation()} style={{
          ...
          <div style={{
            ...
            <div style={{ fontSize: '0.72rem', fontWeight: '700', color: '#9ca3af', marginBottom: '4px' }}>
          ...
          <div style={{ marginBottom: '16px' }}>
            ...
            {selectedItems.map((item: any, i: number) => (
              <div key={i} style={{
                ...
                borderRadius: '12px', border: item.selected ? '1.5px solid #22c55e' : '1px solid #f3f4f6',
                background: item.selected ? '#f0fdf4' : '#fff',
                cursor: 'pointer', transition: 'all 0.15s',
              }} onClick={() => toggleItem(i)}>
                <div style={{ flexShrink: 0 }}>
                  {item.selected ? <CheckSquare size={22} color="#22c55e" /> : <Square size={22} color="#6b7280" />}
                </div>
                <img
                  src={optimizeImageUrl(item.image || item.image_url || '/placeholder.png')}
                  alt=""
                  style={{ width: '44px', height: '44px', borderRadius: '10px', objectFit: 'contain', background: '#ffffff', flexShrink: 0 }}
                />
                ...
                {item.selected && (
                  <div onClick={e => e.stopPropagation()} style={{ display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0 }}>
                    <button
                      ...
                      style={{
                        width: '28px', height: '28px', borderRadius: '7px', border: '1px solid #6b7280',
                        background: '#ffffff', fontWeight: '900', cursor: 'pointer',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
          ...
          <div style={{ marginBottom: '20px' }}>
            <label style={{ display: 'block', fontSize: '0.82rem', fontWeight: '800', color: '#1a1a1a', marginBottom: '8px' }}>
              سبب الاسترجاع <span style={{ color: '#16a34a' }}>*</span>
            </label>
            <textarea
              value={reason}
          ...
          <button
            ...
            style={{
              ...
              background: (submitting || !hasSelection || !reason.trim()) ? '#9ca3af' : 'linear-gradient(135deg, #22c55e, #16a34a)',
  ...
  const customSelectStyles = {
    control: (base: any) => ({ ...base, height: '48px', borderRadius: '12px', border: '1px solid #f3f4f6', background: '#f9fafb', textAlign: 'right', display: 'flex', flexDirection: 'row-reverse' }),
    option: (base: any, state: any) => ({ ...base, padding: '10px 15px', backgroundColor: state.isFocused ? '#f0fdf4' : '#fff', color: '#1a1a1a', cursor: 'pointer', textAlign: 'right' }),
    ...
    <div style={container}>
      ...
      <div style={headerCard}>
        ...
        <div style={headerStatsRow}>
          <div style={headerStatItem}><strong style={{ fontSize: '1.3rem', color: '#1a1a1a' }}>{orders.length}</strong><span style={{ fontSize: '0.7rem', color: '#6b7280' }}>طلبات</span></div>
          <div style={headerDivider} />
          <div style={headerStatItem}><strong style={{ fontSize: '1.3rem', color: '#1a1a1a' }}>{myCars.length}</strong><span style={{ fontSize: '0.7rem', color: '#6b7280' }}>سيارات</span></div>
          <div style={headerDivider} />
          <div style={headerStatItem}><strong style={{ fontSize: '1.3rem', color: '#1a1a1a' }}>{myAddresses.length}</strong><span style={{ fontSize: '0.7rem', color: '#6b7280' }}>عناوين</span></div>
        </div>
      </div>
        ...
        <div style={{ marginTop: '16px' }}>
          ...
          {returnRequests.map((req: any) => {
            const statusConfig: Record<string, { label: string; color: string; bg: string }> = {
              pending:  { label: 'قيد المراجعة', color: '#fb923c', bg: '#ffffff' },
              approved: { label: 'تمت الموافقة', color: '#15803d', bg: '#f0fdf4' },
              rejected: { label: 'مرفوض', color: '#16a34a', bg: '#f0fdf4' },
              refunded: { label: 'تم الاسترجاع', color: '#a78bfa', bg: '#f3f4f6' },
            };
            ...
            return (
              <div key={req.id} style={{
                ...
                {req.admin_notes && (
                  <div style={{ marginTop: '8px', padding: '8px 10px', background: '#f0fdf4', borderRadius: '8px', fontSize: '0.75rem', color: '#15803d', border: '1px solid #166534' }}>
...
const headerCard: any = { background: 'linear-gradient(135deg, #f9fafb 0%, #f0fdf4 55%, #f0fdf4 100%)', borderRadius: '24px', padding: '20px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', border: '1px solid #e5e7eb', boxShadow: '0 12px 34px rgba(0,0,0,0.08), inset 0 1px 0 rgba(0,0,0,0.05)' };
const profileInfoWrap: any = { display: 'flex', alignItems: 'center', gap: '14px' };
const avatarWrap: any = { width: '60px', height: '60px', borderRadius: '18px', background: '#f0fdf4', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '2px solid #166534', flexShrink: 0 };
const titleName: any = { fontSize: '1.1rem', fontWeight: '900', color: '#1a1a1a', marginBottom: '3px' };
const emailText: any = { fontSize: '0.78rem', color: '#6b7280', wordBreak: 'break-all' };
...
// ── Wallet Card styles ──
const walletCard: any = { background: 'linear-gradient(135deg, #f0fdf4, #f0fdf4)', borderRadius: '20px', padding: '16px 18px', marginBottom: '20px', border: '1px solid #16a34a', display: 'flex', alignItems: 'center', gap: '14px', boxShadow: '0 4px 16px rgba(34,197,94,0.18)' };
const walletIconWrap: any = { width: '46px', height: '46px', borderRadius: '14px', background: '#ffffff', border: '1px solid #166534', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, boxShadow: '0 2px 8px rgba(34,197,94,0.12)' };
...
const walletAmount: any = { fontSize: '1.4rem', fontWeight: '900', color: '#15803d', letterSpacing: '-0.5px' };
...
const garageSection: any = { background: '#ffffff', borderRadius: '24px', padding: '18px', marginBottom: '20px', border: '1px solid #f3f4f6', boxShadow: '0 2px 12px rgba(0,0,0,0.03)' };
const garageScroll: any = { display: 'flex', gap: '12px', overflowX: 'auto', padding: '4px 0 8px', scrollbarWidth: 'none' };
const carCard: any = { flex: '0 0 175px', background: '#f9fafb', border: '1px solid #f3f4f6', borderRadius: '16px', padding: '12px', display: 'flex', alignItems: 'center', gap: '10px', position: 'relative' };
const carIconWrap: any = { width: '38px', height: '38px', borderRadius: '12px', background: '#ffffff', border: '1px solid #f0fdf4', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 };
const carDetails: any = { flex: 1, overflow: 'hidden' };
const carNameText: any = { fontSize: '0.82rem', fontWeight: '800', color: '#1a1a1a', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' };
const carYearText: any = { fontSize: '0.72rem', color: '#374151', marginTop: '2px' };
const carDeleteBtn: any = { position: 'absolute', top: '8px', left: '8px', color: '#16a34a', background: 'none', border: 'none', cursor: 'pointer', padding: '2px' };
const tabBar: any = { display: 'flex', gap: '8px', marginBottom: '16px', background: '#ffffff', borderRadius: '16px', padding: '6px', border: '1px solid #f3f4f6' };
const tabBtn = (active: boolean): any => ({ flex: 1, padding: '10px 6px', borderRadius: '12px', border: 'none', background: active ? '#f0fdf4' : 'transparent', color: active ? '#16a34a' : '#6b7280', fontWeight: active ? '900' : '600', fontSize: '0.8rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px', transition: '0.2s' });
const gridSplit: any = { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(min(100%, 400px), 1fr))', gap: '20px' };
const mainCard: any = { background: '#ffffff', borderRadius: '24px', padding: '18px', marginBottom: '20px', border: '1px solid #f3f4f6', boxShadow: '0 2px 12px rgba(0,0,0,0.02)', width: '100%', boxSizing: 'border-box' as const };
...
const dateText: any = { fontSize: '0.72rem', color: '#9ca3af' };
...
const saveActionBtn: any = { padding: '12px', background: '#22c55e', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: 'bold', cursor: 'pointer', fontSize: '0.9rem', marginTop: '4px', width: '100%' };
const miniAddBtn: any = { width: '32px', height: '32px', borderRadius: '10px', background: '#f0fdf4', color: '#22c55e', border: '1px solid #166534', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '0.85rem', fontWeight: '900' };
const compactAddBox: any = { background: '#f9fafb', padding: '14px', borderRadius: '16px', marginBottom: '14px', border: '1px dashed #16a34a' };
...
const addressCard: any = { padding: '12px', border: '1px solid #f3f4f6', borderRadius: '14px', display: 'flex', alignItems: 'center', gap: '10px', background: '#f9fafb' };
...
const addrFull: any = { fontSize: '0.73rem', color: '#6b7280', marginTop: '2px' };
const addrDelBtn: any = { color: '#16a34a', background: 'none', border: 'none', cursor: 'pointer', padding: '4px', flexShrink: 0 };
const elegantLogout: any = { width: '100%', padding: '14px', background: 'none', border: '1px solid #f0fdf4', color: '#16a34a', borderRadius: '16px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', fontWeight: '900', fontSize: '0.9rem', cursor: 'pointer', marginTop: '8px' };
