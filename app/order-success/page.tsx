...
//
// EasyKash redirects here after payment:
//   /order-success?status=success&providerRefNum=XXX&customerReference=YYY
//
// We poll Supabase until the callback has created the real order,
// then show the invoice.

import { useEffect, useState, useRef } from 'react';
import { useSearchParams, useRouter } from 'next/navigation';
import { supabase } from '@/app/lib/supabase';
import { CheckCircle, XCircle, Loader2, Download, FileText, ExternalLink, Home } from 'lucide-react';
import Link from 'next/link';
import toast from 'react-hot-toast';

const POLL_INTERVAL_MS = 2000;  // check every 2 seconds
const POLL_MAX_TRIES   = 30;    // give up after 1 minute

export default function OrderSuccessPage() {
  const params      = useSearchParams();
  const router      = useRouter();

  const status    = params.get('status');          // 'success' | 'pending' | 'failed'
  ...
  useEffect(() => {
    // If EasyKash says payment failed, show error immediately
    if (status === 'failed') {
      setPhase('payment_failed');
      return;
    }

    // Otherwise poll for the order that the callback will create
    pollForOrder();
  ...
  async function pollForOrder() {
    ...
    try {
      // Look up order by easykash_ref (providerRefNum) or customerReference
      let query = supabase.from('orders').select('*');

      if (provRef) {
        query = query.eq('easykash_ref', provRef);
      } else if (custRef) {
        // fallback: customerReference is last 8 digits of orderId-ish number
        // we stored it in pending_orders — but after callback the real order is created
        // Try to match by a temp reference stored during checkout
        query = query.eq('easykash_customer_ref', custRef);
      } else {
        if (intervalRef.current) clearInterval(intervalRef.current);
        setPhase('failed');
        return;
  ...
  if (phase === 'payment_failed') {
    return (
      <div style={centerStyle}>
        <div style={{ ...pollingCard, borderColor: '#fee2e2' }}>
          <XCircle size={52} color="#dc2626" style={{ marginBottom: '20px' }} />
          <h2 style={{ fontWeight: '900', fontSize: '1.4rem', marginBottom: '8px', color: '#dc2626' }}>
            لم يتم إتمام الدفع
          </h2>
  ...
  // ── Timeout / can't find order ────────────────────────────────────────────
  if (phase === 'failed') {
    return (
      <div style={centerStyle}>
        <div style={{ ...pollingCard, borderColor: '#fef3c7' }}>
          <div style={{ fontSize: '3rem', marginBottom: '16px' }}>⏳</div>
          <h2 style={{ fontWeight: '900', fontSize: '1.3rem', marginBottom: '8px', color: '#1a1a1a' }}>
            تأخر ظهور تأكيد الطلب
          </h2>
          <p style={{ color: '#888', fontSize: '0.88rem', lineHeight: '1.6', marginBottom: '24px' }}>
            لو تم خصم المبلغ، الطلب هيظهر في حسابك خلال دقيقة.
            <br />
            تواصل معنا إذا استمرت المشكلة.
          </p>
          <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
            <button onClick={() => { setPhase('polling'); setTries(0); pollForOrder(); }}
    ...
    return (
      <div style={{ direction: 'rtl', padding: '30px 20px', maxWidth: '820px', margin: '0 auto', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
        ...
        <div style={{ background: 'linear-gradient(135deg, #0f172a, #14532d)', borderRadius: '20px', padding: '28px 30px', textAlign: 'center', marginBottom: '20px', color: '#fff' }}>
          ...
          {order.easykash_payment_method && (
            <p style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.78rem', marginTop: '4px' }}>
              طريقة الدفع: {order.easykash_payment_method}
            </p>
          )}
        </div>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '22px', flexWrap: 'wrap' }}>
          ...
          <button onClick={() => router.push('/')}
            style={{ flex: 1, minWidth: '140px', padding: '13px 20px', background: '#fff', color: '#1a1a1a', border: '1.5px solid #e5e5e5', borderRadius: '14px', fontWeight: '700', fontSize: '0.9rem', cursor: 'pointer' }}>
            العودة للرئيسية
          </button>
        </div>
        ...
        <div id="invoice-preview" style={{ backgroundColor: '#fff', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.08)', border: '1px solid #f0f0f0' }}>
          ...
          {/* Header */}
          <div style={{ background: 'linear-gradient(135deg, #0f172a 0%, #1e293b 60%, #0f4c2a 100%)', padding: '32px 40px', position: 'relative', overflow: 'hidden' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', position: 'relative', zIndex: 1 }}>
              ...
              <div style={{ textAlign: 'left' }}>
                <div style={{ fontSize: '2.4rem', fontWeight: '900', color: '#22c55e', letterSpacing: '-1px', lineHeight: 1 }}>ORDER</div>
                <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.78rem', fontWeight: '700', marginTop: '4px' }}>#{orderNum}</div>
              </div>
            </div>
            <div style={{ marginTop: '16px', display: 'flex', alignItems: 'center', gap: '10px', position: 'relative', zIndex: 1 }}>
              ...
              <span style={{ color: 'rgba(255,255,255,0.4)', fontSize: '0.76rem' }}>{orderDate}</span>
            </div>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', borderBottom: '1px solid #f0f0f0' }}>
            {[{ label: 'رقم الطلب', value: `#${orderNum}` }, { label: 'تاريخ الطلب', value: orderDate }, { label: 'عدد المنتجات', value: `${items.length} منتج` }].map((m, i) => (
              <div key={i} style={{ padding: '16px 20px', borderRight: i < 2 ? '1px solid #f0f0f0' : 'none' }}>
                <div style={{ fontSize: '0.66rem', color: '#aaa', fontWeight: '700', letterSpacing: '0.5px', textTransform: 'uppercase', marginBottom: '4px' }}>{m.label}</div>
          ...
          <div style={{ background: 'linear-gradient(135deg, #0f172a, #1e293b)', padding: '22px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ backgroundColor: 'rgba(34,197,94,0.15)', border: '1px solid rgba(34,197,94,0.3)', borderRadius: '8px', padding: '5px 12px', color: '#22c55e', fontSize: '0.7rem', fontWeight: '800', letterSpacing: '1px' }}>
              ORDER #{orderNum}
            </div>
          </div>
        </div>
...
const centerStyle: any = {
  ...
  minHeight: '80vh', padding: '20px', direction: 'rtl',
};
const pollingCard: any = {
