'use client';
import { Suspense } from 'react'; // استيراد Suspense ضروري لحل مشكلة الـ Build
import { useSearchParams } from 'next/navigation';
import { motion } from 'framer-motion';
import { CheckCircle, Package, Home, ShoppingBag, Truck } from 'lucide-react';
import Link from 'next/link';

// فصل محتوى الصفحة في مكون فرعي لاستخدامه داخل Suspense
...
// المكون الرئيسي المغلف بالـ Suspense
export default function OrderSuccessPage() {
  return (
    <div style={container}>
      <Suspense fallback={<div style={{textAlign:'center'}}><Package className="animate-bounce" /> جاري التحميل...</div>}>
        <SuccessContent />
      </Suspense>
    </div>
  );
...
const primaryBtn: any = { background: '#1a1a1a', color: '#fff', padding: '15px', borderRadius: '15px', textDecoration: 'none', fontWeight: 'bold', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' };
const secondaryBtn: any = { background: '#27ae60', color: '#fff', padding: '15px', borderRadius: '15px', textDecoration: 'none', fontWeight: 'bold', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '10px' };
