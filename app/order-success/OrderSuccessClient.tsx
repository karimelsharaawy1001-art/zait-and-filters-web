...
export default function OrderSuccessClient() {
  ...
  if (phase === 'payment_failed') return (
    <div style={centerStyle}>
      <div style={{ ...card, borderColor: '#f0fdf4' }}>
        <XCircle size={52} color="#16a34a" style={{ marginBottom: '20px' }} />
        <h2 style={{ fontWeight: '900', fontSize: '1.4rem', marginBottom: '8px', color: '#16a34a' }}>لم يتم إتمام الدفع</h2>
        <p style={{ color: '#9ca3af', fontSize: '0.9rem', marginBottom: '24px' }}>لم يتم خصم أي مبلغ من حسابك. يمكنك المحاولة مرة أخرى.</p>
        <button onClick={() => router.push('/checkout')}
          style={{ padding: '12px 28px', background: '#ffffff', color: '#1a1a1a', border: '1px solid #e5e7eb', borderRadius: '12px', fontWeight: '800', cursor: 'pointer', fontSize: '0.95rem' }}>
          العودة للدفع
        </button>
      </div>
    </div>
  ...
  if (phase === 'failed') return (
    <div style={centerStyle}>
      <div style={{ ...card, borderColor: '#f3f4f6' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap' }}>
          ...
          <button onClick={() => router.push('/')}
            style={{ padding: '11px 22px', background: '#ffffff', color: '#9ca3af', border: 'none', borderRadius: '12px', fontWeight: '700', cursor: 'pointer' }}>
            الرئيسية
          </button>
        </div>
      </div>
  ...
  if (phase === 'found' && order) {
    ...
    return (
      <div style={{ direction: 'rtl', padding: '30px 20px', maxWidth: '820px', margin: '0 auto', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
        ...
        {/* Success banner */}
        <div style={{ background: 'linear-gradient(135deg, #e5e7eb, #166534)', borderRadius: '20px', padding: '28px 30px', textAlign: 'center', marginBottom: '20px', color: '#fff' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', marginBottom: '22px', flexWrap: 'wrap' }}>
          <button onClick={handleDownloadInvoice} disabled={isDownloading}
            style={{ flex: 1, minWidth: '180px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', padding: '13px 20px', background: isDownloading ? '#9ca3af' : 'linear-gradient(135deg, #22c55e, #16a34a)', color: '#fff', border: 'none', borderRadius: '14px', fontWeight: '800', fontSize: '0.9rem', cursor: isDownloading ? 'not-allowed' : 'pointer' }}>
          ...
          <button onClick={() => router.push('/')}
            style={{ flex: 1, minWidth: '140px', padding: '13px 20px', background: '#ffffff', color: '#1a1a1a', border: '1.5px solid #e5e7eb', borderRadius: '14px', fontWeight: '700', fontSize: '0.9rem', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
            <Home size={16} /> الرئيسية
          </button>
        </div>

        {/* Invoice */}
        <div id="invoice-preview" style={{ backgroundColor: '#ffffff', borderRadius: '16px', overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,0.08)', border: '1px solid #f3f4f6' }}>
          {/* Header */}
          <div style={{ background: 'linear-gradient(135deg, #e5e7eb 0%, #6b7280 60%, #0f4c2a 100%)', padding: '32px 40px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
              ...
              <div style={{ textAlign: 'left' }}>
                ...
                <div style={{ color: 'rgba(0,0,0,0.2)', fontSize: '0.78rem', fontWeight: '700', marginTop: '4px' }}>#{orderNum}</div>
              </div>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', borderBottom: '1px solid #f3f4f6' }}>
            {[{ label: 'رقم الطلب', value: `#${orderNum}` }, { label: 'تاريخ الطلب', value: orderDate }, { label: 'عدد المنتجات', value: `${items.length} منتج` }].map((m, i) => (
              <div key={i} style={{ padding: '16px 20px', borderRight: i < 2 ? '1px solid #f3f4f6' : 'none' }}>
                <div style={{ fontSize: '0.66rem', color: '#6b7280', fontWeight: '700', textTransform: 'uppercase', marginBottom: '4px' }}>{m.label}</div>
          ...
          <div style={{ padding: '28px 40px' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '24px' }}>
              <div style={{ backgroundColor: '#f9fafb', borderRadius: '12px', padding: '18px', border: '1px solid #f3f4f6' }}>
                <div style={{ fontSize: '0.66rem', fontWeight: '900', color: '#9ca3af', textTransform: 'uppercase', marginBottom: '10px' }}>بيانات العميل</div>
                ...
                {order.customer_email && <div style={{ fontSize: '0.76rem', color: '#9ca3af' }}>{order.customer_email}</div>}
              </div>
              <div style={{ backgroundColor: '#f9fafb', borderRadius: '12px', padding: '18px', border: '1px solid #f3f4f6' }}>
                <div style={{ fontSize: '0.66rem', fontWeight: '900', color: '#9ca3af', textTransform: 'uppercase', marginBottom: '10px' }}>عنوان التوصيل</div>
            ...
            <div style={{ marginBottom: '22px' }}>
              <div style={{ fontSize: '0.66rem', fontWeight: '900', color: '#9ca3af', textTransform: 'uppercase', marginBottom: '10px' }}>تفاصيل المنتجات</div>
              <div style={{ backgroundColor: '#ffffff', borderRadius: '10px 10px 0 0', padding: '10px 14px', display: 'grid', gridTemplateColumns: '2.5fr 0.7fr 1fr 1fr' }}>
              ...
              {items.map((item: any, i: number) => (
                <div key={i} style={{ display: 'grid', gridTemplateColumns: '2.5fr 0.7fr 1fr 1fr', padding: '11px 14px', backgroundColor: i % 2 === 0 ? '#fff' : '#f9fafb', borderBottom: '1px solid #f3f4f6', borderRight: '1px solid #f3f4f6', borderLeft: '1px solid #f3f4f6', alignItems: 'center' }}>
                  <div>
                  ...
                  <div style={{ textAlign: 'center', fontSize: '0.83rem', fontWeight: '700', color: '#374151' }}>{parseFloat(item.price).toLocaleString('ar-EG')} ج.م</div>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <div style={{ width: '260px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px dashed #e5e7eb', fontSize: '0.84rem' }}>
                ...
                {discount > 0 && (
                  <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', borderBottom: '1px dashed #e5e7eb', fontSize: '0.84rem' }}>
                    ...
                    <span style={{ color: '#16a34a', fontWeight: '800' }}>- {discount.toLocaleString('ar-EG')} ج.م</span>
                  </div>
                )}
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: '10px', padding: '12px 16px', background: 'linear-gradient(135deg, #e5e7eb, #6b7280)', borderRadius: '12px' }}>
          ...
          {/* Footer */}
          <div style={{ background: 'linear-gradient(135deg, #e5e7eb, #6b7280)', padding: '22px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
...
const card: any = { backgroundColor: '#ffffff', borderRadius: '20px', padding: '48px 40px', textAlign: 'center', maxWidth: '440px', width: '100%', boxShadow: '0 10px 40px rgba(0,0,0,0.08)', border: '1px solid #f3f4f6' };