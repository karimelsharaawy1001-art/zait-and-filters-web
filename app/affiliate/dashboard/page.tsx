  ...
  if (loading) {
    return (
      <div style={loaderContainer}>
        <Loader2 className="animate-spin" size={50} color="#22c55e" />
        <p style={{ color: '#6b7280', marginTop: '20px', fontSize: '1.1rem' }}>
  ...
  return (
    <div style={container}>
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .copy-btn:hover { background: #22c55e !important; color: #fff !important; }
      ...
      <div style={statsGrid} className="slide-in">
        <div style={{ ...statCard, borderTop: '4px solid #22c55e' }} className="stat-card">
          <div style={statIconContainer}><DollarSign size={28} color="#22c55e" /></div>
        ...
        <div style={{ ...statCard, borderTop: '4px solid #3b82f6' }} className="stat-card">
          <div style={{ ...statIconContainer, background: '#f9fafb' }}><Eye size={28} color="#3b82f6" /></div>
          <div style={statContent}>
        ...
        <div style={{ ...statCard, borderTop: '4px solid #f59e0b' }} className="stat-card">
          <div style={{ ...statIconContainer, background: '#f3f4f6' }}><ShoppingBag size={28} color="#f59e0b" /></div>
          <div style={statContent}>
        ...
        <div style={{ ...statCard, borderTop: '4px solid #8b5cf6' }} className="stat-card">
          <div style={{ ...statIconContainer, background: '#f3f4f6' }}><Wallet size={28} color="#8b5cf6" /></div>
          <div style={statContent}>
        ...
        <div style={{ ...statCard, borderTop: '4px solid #f59e0b' }} className="stat-card">
          <div style={{ ...statIconContainer, background: '#f3f4f6' }}><DollarSign size={28} color="#f59e0b" /></div>
          <div style={statContent}>
      ...
      <div style={toolsSection} className="slide-in">
        ...
        <div style={toolCard}>
          <div style={toolHeader}><LinkIcon size={20} color="#22c55e" /><span>رابط الإحالة الخاص بك</span></div>
          <div style={codeBox}>
        ...
        <div style={toolCard}>
          <div style={toolHeader}><Ticket size={20} color="#22c55e" /><span>كود الخصم الحصري (خصم 5%)</span></div>
      ...
      <div style={withdrawalSection} className="slide-in">
        ...
        <div style={withdrawalCard}>
          <p style={{ fontSize: '0.85rem', color: '#6b7280', marginBottom: '18px', fontWeight: '600' }}>
            أدخل أرقام حساباتك لاستلام العمولات. يمكنك إدخال واحد أو الاثنين معاً.
          </p>

          {/* InstaPay */}
          <div style={{ ...inputGroup, background: '#f0fdf4', border: '1.5px solid #166534', borderRadius: '14px', padding: '16px', marginBottom: '14px' }}>
            ...
            <input
              ...
              style={{ ...input, borderColor: withdrawalSettings.instapay_phone ? '#22c55e' : '#e5e7eb' }}
              maxLength={11}
              dir="ltr"
            />
          ...
          {/* E-Wallet */}
          <div style={{ ...inputGroup, background: '#f9fafb', border: '1.5px solid #e5e7eb', borderRadius: '14px', padding: '16px', marginBottom: '20px' }}>
            ...
            <input
              ...
              style={{ ...input, borderColor: withdrawalSettings.wallet_phone ? '#3b82f6' : '#e5e7eb' }}
              maxLength={11}
              dir="ltr"
            />
      ...
      <div style={commissionsSection} className="slide-in">
        ...
        {stats.recent_commissions.length > 0 ? (
              ...
              <tbody>
                {stats.recent_commissions.map((comm: any) => {
                  ...
                  const statusConfig: Record<string, { bg: string; color: string; icon: any; label: string }> = {
                    ...
                    in_review: { bg: '#f3f4f6', color: '#fbbf24', icon: <Clock size={13}/>,       label: daysLeft && daysLeft > 0 ? `${daysLeft} يوم متبقي` : 'جاهزة قريباً' },
                    available: { bg: '#f0fdf4', color: '#059669', icon: <CheckCircle size={13}/>, label: 'جاهزة للصرف' },
                    paid:      { bg: '#ffffff', color: '#9ca3af', icon: <CheckCircle size={13}/>, label: 'تم الدفع' },
                  };
                  ...
                  return (
                    <tr key={comm.id} style={tableRow}>
                      ...
                      <td style={{...td, color: '#22c55e', fontWeight: 'bold'}}>+{parseFloat(comm.commission_amount).toFixed(2)} ج.م</td>
                      <td style={td}>
          ...
          <div style={emptyState}>
            <BarChart3 size={50} color="#374151" />
...
// Styles
const container: any = { minHeight: '100vh', background: 'linear-gradient(135deg, #f9fafb 0%, #e8edf2 100%)', padding: '40px 20px', direction: 'rtl' };
const loaderContainer: any = { minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: '#f9fafb' };
...
const subtitle: any = { color: '#6b7280', fontSize: '1.05rem', marginTop: '5px' };
const tierBadge: any = { display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 24px', borderRadius: '50px', border: '2px solid #e5e7eb', fontWeight: 'bold', fontSize: '0.95rem', boxShadow: '0 4px 10px rgba(0,0,0,0.1)' };
const logoutBtn: any = { display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', background: '#ffffff', border: '1px solid #e5e7eb', borderRadius: '12px', color: '#6b7280', fontWeight: 'bold', cursor: 'pointer', transition: '0.3s' };
const tierProgress: any = { maxWidth: '1400px', margin: '0 auto 30px', background: '#ffffff', padding: '20px', borderRadius: '16px', boxShadow: '0 2px 10px rgba(0,0,0,0.06)' };
const tierProgressHeader: any = { display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px', fontWeight: 'bold', color: '#e5e7eb' };
const progressBar: any = { width: '100%', height: '12px', background: '#f3f4f6', borderRadius: '10px', overflow: 'hidden' };
...
const statCard: any = { background: '#ffffff', borderRadius: '20px', padding: '30px', boxShadow: '0 2px 10px rgba(0,0,0,0.06)', transition: '0.4s', display: 'flex', alignItems: 'center', gap: '20px' };
...
const statLabel: any = { color: '#6b7280', fontSize: '0.9rem', marginBottom: '8px' };
const statValue: any = { fontSize: '2rem', fontWeight: '900', color: '#e5e7eb', margin: 0 };
const statGrowth: any = { color: '#22c55e', fontSize: '0.85rem', marginTop: '5px', fontWeight: '600' };
const pendingNotice: any = { maxWidth: '1400px', margin: '0 auto 30px', background: '#ffffff', padding: '20px', borderRadius: '16px', border: '1px solid #f3f4f6', display: 'flex', alignItems: 'center', gap: '15px' };
const toolsSection: any = { maxWidth: '1400px', margin: '0 auto 40px', background: '#ffffff', borderRadius: '20px', padding: '35px', boxShadow: '0 2px 10px rgba(0,0,0,0.06)' };
...
const toolCard: any = { background: '#f9fafb', padding: '25px', borderRadius: '16px', marginBottom: '20px', border: '1px solid #e5e7eb' };
...
const codeBox: any = { display: 'flex', gap: '12px', background: '#ffffff', padding: '18px', borderRadius: '12px', border: '1px solid #e5e7eb', alignItems: 'center' };
const codeText: any = { flex: 1, fontSize: '0.9rem', wordBreak: 'break-all', color: '#6b7280', fontFamily: 'monospace' };
const copyBtn: any = { padding: '10px 16px', background: '#f3f4f6', border: '1px solid #e5e7eb', borderRadius: '10px', cursor: 'pointer', transition: '0.3s', display: 'flex', alignItems: 'center', gap: '6px', flexShrink: 0, fontWeight: 'bold', color: '#6b7280' };
const toolHint: any = { marginTop: '12px', fontSize: '0.85rem', color: '#6b7280', lineHeight: '1.6' };
const promoCodeDisplay: any = { display: 'flex', gap: '12px', alignItems: 'center' };
const promoCodeBadge: any = { flex: 1, background: 'linear-gradient(135deg, #22c55e 0%, #229954 100%)', color: '#fff', padding: '20px 30px', borderRadius: '12px', fontSize: '1.8rem', fontWeight: '900', textAlign: 'center', letterSpacing: '3px', fontFamily: 'monospace', boxShadow: '0 4px 15px rgba(39, 174, 96, 0.3)' };
const withdrawalSection: any = { maxWidth: '1400px', margin: '0 auto 40px', background: '#ffffff', borderRadius: '20px', padding: '35px', boxShadow: '0 2px 10px rgba(0,0,0,0.06)' };
const withdrawalCard: any = { background: '#f9fafb', padding: '25px', borderRadius: '16px', border: '1px solid #e5e7eb' };
...
const input: any = { width: '100%', padding: '14px', borderRadius: '12px', border: '1px solid #e5e7eb', fontSize: '1rem', outline: 'none', background: '#ffffff' };
const saveButton: any = { display: 'flex', alignItems: 'center', gap: '10px', padding: '14px 30px', background: '#22c55e', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: 'bold', cursor: 'pointer', transition: '0.3s', fontSize: '1rem', boxShadow: '0 4px 10px rgba(39, 174, 96, 0.3)' };
const commissionsSection: any = { maxWidth: '1400px', margin: '0 auto', background: '#ffffff', borderRadius: '20px', padding: '35px', boxShadow: '0 2px 10px rgba(0,0,0,0.06)' };
const refreshBtn: any = { display: 'flex', alignItems: 'center', gap: '8px', padding: '10px 20px', background: '#f9fafb', border: '1px solid #e5e7eb', borderRadius: '12px', cursor: 'pointer', fontWeight: 'bold', color: '#6b7280', transition: '0.3s' };
const tableWrapper: any = { overflowX: 'auto', borderRadius: '12px', border: '1px solid #e5e7eb' };
const table: any = { width: '100%', borderCollapse: 'collapse' };
const tableHeader: any = { background: '#f9fafb' };
const th: any = { padding: '16px 20px', textAlign: 'right', fontWeight: 'bold', color: '#6b7280', fontSize: '0.9rem', borderBottom: '2px solid #e5e7eb' };
const tableRow: any = { borderBottom: '1px solid #f3f4f6', transition: '0.2s' };
...
const emptyText: any = { fontSize: '1.2rem', color: '#6b7280', fontWeight: 'bold', marginTop: '20px', marginBottom: '8px' };
const emptyHint: any = { color: '#6b7280', fontSize: '0.95rem' };