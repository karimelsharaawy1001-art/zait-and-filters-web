...
function RelatedProductCard({ p }: { p: any }) {
  ...
  const { addToCart } = useCart(); // ربط دالة الإضافة للسلة في الكروت
...
export default function ProductDetails({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [qty, setQty] = useState(1);
  const { addToCart } = useCart(); // ربط دالة الإضافة للسلة في الصفحة
    ...
    async function fetchProductAndRelated() {
      ...
      if (mainProduct) {
        ...
        const { data: related } = await supabase.from('products')
          .select('*')
          .eq('car_make', mainProduct.car_make)
          .eq('car_model', mainProduct.car_model)
  ...
  if (loading) return <div style={loaderWrapper}><Loader2 className="animate-spin" color="#27ae60" size={50}/></div>;
