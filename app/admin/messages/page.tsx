    ...
    <div style={{ direction: 'rtl', color: '#1a1a1a', fontFamily: 'sans-serif', padding: 'clamp(12px, 3vw, 20px)', backgroundColor: '#f8f9fa', minHeight: '100vh' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
        <h1 style={{ color: '#27ae60', fontWeight: '900', fontSize: 'clamp(1.1rem, 5vw, 1.5rem)', margin: 0 }}>
          رسائل العملاء
          <span style={{ fontSize: '0.85rem', color: '#6b7280', marginRight: '10px', fontWeight: '400' }}>
      ...
      <div className="msg-desktop-table" style={{ backgroundColor: '#fff', borderRadius: '15px', border: '1px solid #eee', overflow: 'hidden', boxShadow: '0 2px 12px rgba(0,0,0,0.04)' }}>
        <div style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch' as any }}>
          <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'right', minWidth: '600px' }}>
            <thead>
              <tr style={{ backgroundColor: '#ffffff', color: '#27ae60', borderBottom: '2px solid #eee' }}>
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={5} style={{ textAlign: 'center', padding: '50px', color: '#6b7280' }}>جاري التحميل...</td></tr>
              ) : messages.length === 0 ? (
                <tr><td colSpan={5} style={{ textAlign: 'center', padding: '50px', color: '#6b7280' }}>لا توجد رسائل بعد</td></tr>
              ) : messages.map((msg) => (
                <tr key={msg.id} style={{ borderBottom: '1px solid #f0f0f0', backgroundColor: msg.status === 'new' || !msg.status ? '#f0fdf4' : '#fff' }}>
                  <td style={tdStyle}>
                    <span style={{ fontSize: '0.85rem', color: '#9ca3af' }}>{new Date(msg.created_at).toLocaleDateString('ar-EG')}</span>
                  </td>
                  <td style={tdStyle}>
                    <div style={{ fontWeight: 'bold', color: '#1a1a1a' }}>{msg.full_name}</div>
                    <div style={{ fontSize: '0.8rem', color: '#9ca3af', marginBottom: '4px' }}>{msg.phone}</div>
                    {msg.email && <div style={{ fontSize: '0.75rem', color: '#6b7280', marginBottom: '8px' }}>{msg.email}</div>}
                  </td>
                  <td style={tdStyle}>
                    <div style={{ fontSize: '0.9rem', color: '#6b7280', maxWidth: '350px' }}>{msg.message}</div>
                  </td>
                    <td style={tdStyle}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', flexWrap: 'wrap' }}>
                        {waLink(msg.phone, msg.full_name, msg.message) && (
                          <a href={waLink(msg.phone, msg.full_name, msg.message)!} target="_blank" rel="noopener noreferrer"
                        ...
                        <select value={msg.status || 'new'} onChange={(e) => updateStatus(msg.id, e.target.value)}
                          style={{ backgroundColor: '#f9f9f9', color: msg.status === 'new' || !msg.status ? '#27ae60' : '#9ca3af', border: '1px solid #ddd', borderRadius: '8px', padding: '6px 10px', fontSize: '0.85rem', cursor: 'pointer' }}>
                    ...
                    <td style={tdStyle}>
                      <button onClick={() => deleteMessage(msg.id)}
                        style={{ color: '#16a34a', background: 'none', border: '1px solid #fee2e2', borderRadius: '8px', padding: '6px 12px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.8rem' }}>
                        حذف
                      </button>
                    </td>
                </tr>
      ...
      <div className="msg-mobile-cards" style={{ display: 'none', flexDirection: 'column', gap: '10px' }}>
        {loading ? (
          <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280', background: '#fff', borderRadius: '12px' }}>جاري التحميل...</div>
        ) : messages.length === 0 ? (
          <div style={{ textAlign: 'center', padding: '40px', color: '#6b7280', background: '#fff', borderRadius: '12px' }}>لا توجد رسائل بعد</div>
        ) : messages.map((msg) => (
          <div key={msg.id} style={{ backgroundColor: msg.status === 'new' || !msg.status ? '#f0fdf4' : '#fff', borderRadius: '12px', border: `1px solid ${msg.status === 'new' || !msg.status ? '#bbf7d0' : '#eee'}`, padding: '14px', boxShadow: '0 1px 6px rgba(0,0,0,0.04)' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '8px', gap: '8px' }}>
              <div>
                ...
                <div style={{ fontSize: '0.78rem', color: '#9ca3af', marginTop: '2px' }}>{msg.phone}</div>
                {msg.email && <div style={{ fontSize: '0.72rem', color: '#6b7280' }}>{msg.email}</div>}
              </div>
              <div style={{ fontSize: '0.75rem', color: '#6b7280', whiteSpace: 'nowrap', flexShrink: 0 }}>
                {new Date(msg.created_at).toLocaleDateString('ar-EG')}
              </div>
            </div>
            {/* Message */}
            <div style={{ fontSize: '0.88rem', color: '#6b7280', lineHeight: '1.5', background: '#f9fafb', borderRadius: '8px', padding: '8px 10px', marginBottom: '10px', border: '1px solid #f0f0f0' }}>
              {msg.message}
            </div>
            {/* Actions */}
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <select value={msg.status || 'new'} onChange={(e) => updateStatus(msg.id, e.target.value)}
                style={{ flex: 1, minWidth: '110px', backgroundColor: '#f9f9f9', color: msg.status === 'new' || !msg.status ? '#27ae60' : '#9ca3af', border: '1px solid #ddd', borderRadius: '8px', padding: '7px 10px', fontSize: '0.82rem', cursor: 'pointer' }}>
                <option value="new">جديدة</option>
                <option value="read">تمت القراءة</option>
                <option value="replied">تم الرد</option>
              </select>
              <button onClick={() => deleteMessage(msg.id)}
                style={{ color: '#16a34a', background: 'none', border: '1px solid #fee2e2', borderRadius: '8px', padding: '7px 12px', cursor: 'pointer', fontWeight: 'bold', fontSize: '0.8rem' }}>
                حذف
              </button>
            </div>
          </div>
