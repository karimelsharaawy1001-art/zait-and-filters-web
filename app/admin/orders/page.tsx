...
function Dropdown({ label, options, value, onChange, disabled }: {
  ...
  return (
    <div ref={ref} style={{ position: 'relative' }}>
      <button type="button" onClick={() => !disabled && setOpen(!open)} style={{ width: '100%', height: '46px', padding: '0 14px', backgroundColor: disabled ? '#f5f5f5' : '#fff', border: '1px solid #e5e5e5', borderRadius: '10px', fontSize: '0.88rem', color: value ? '#1a1a1a' : '#999', cursor: disabled ? 'not-allowed' : 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', boxSizing: 'border-box' as any }}>
      ...
      {open && options.length > 0 && (
        <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 3000, backgroundColor: '#fff', border: '1px solid #e5e5e5', borderRadius: '10px', boxShadow: '0 8px 24px rgba(0,0,0,0.12)', maxHeight: '200px', overflowY: 'auto', marginTop: '4px' }}>
          ...
          {options.map(opt => (
            <div key={opt} onClick={() => { onChange(opt); setOpen(false); }} style={{ ...ddItem, backgroundColor: value === opt ? '#f0fdf4' : '#fff', fontWeight: value === opt ? '800' : '600', color: value === opt ? '#16a34a' : '#1a1a1a' }}>{opt}</div>
          ))}
        </div>
      )}
    </div>
...
function NewOrderModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  useEffect(() => {
    const t = setTimeout(() => { if (productSearch.trim().length >= 1 || filterMake || filterCategory) doSearch(); else setSearchResults([]); }, 300);
  ...
  async function searchCustomers(q: string) {
    ...
    const { data } = await supabase.from('profiles').select('id, full_name, phone_number').or(`full_name.ilike.%${q}%,phone_number.ilike.%${q}%`).limit(8);
    setCustomerResults(data || []); setShowCustomerResults(true);
  }
  function addProduct(p: any) {
    ...
    setOrderItems(prev => {
      ...
      return [...prev, { id: p.id, name: p.name, price, quantity: 1, image_url: p.image_url, brand: p.brand }];
    });
    setShowResults(false); setProductSearch(''); toast.success(`تمت إضافة "${p.name}"`);
  }
  ...
  async function submitOrder() {
    ...
    try {
      const { error } = await supabase.from('orders').insert({
        ...
        customer_address: manualAddress, city: manualCity,
        items: orderItems.map(i => ({ id: i.id, name: i.name, price: i.price, quantity: i.quantity, image_url: i.image_url })),
        total_price: total, shipping_cost: shippingCost, discount_applied: discountVal,
        payment_method: paymentMethod, payment_status: 'pending',
        status: 'processing', created_at: new Date().toISOString(),
      });
      if (error) throw error;
      toast.success('تم إنشاء الطلب بنجاح ✅'); onCreated(); onClose();
    } catch (err: any) { toast.error('خطأ: ' + err.message); } finally { setSubmitting(false); }
  }
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.65)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 2000, padding: '20px', backdropFilter: 'blur(4px)' }}>
      <div style={{ background: '#f8f9fa', width: '100%', maxWidth: '1060px', maxHeight: '93vh', overflowY: 'auto', borderRadius: '24px', padding: '28px', boxShadow: '0 25px 60px rgba(0,0,0,0.25)', direction: 'rtl' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '22px', borderBottom: '1px solid #e8e8e8', paddingBottom: '16px' }}>
          <h2 style={{ margin: 0, fontSize: '1.4rem', fontWeight: '900', color: '#1a1a1a', display: 'flex', alignItems: 'center', gap: '10px' }}><Package size={22} color="#22c55e" /> إنشاء طلب جديد يدوياً</h2>
          <button onClick={onClose} style={{ background: '#fff', border: '1px solid #eee', borderRadius: '50%', width: '38px', height: '38px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={19} color="#666" /></button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 310px', gap: '18px', alignItems: 'flex-start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
            <div style={nomCard}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '10px', marginBottom: '12px' }}>
                ...
                <button onClick={() => { setFilterMake(''); setFilterModel(''); setFilterYear(''); setFilterCategory(''); setFilterSubcategory(''); }} style={{ height: '46px', background: '#fee2e2', color: '#dc2626', border: 'none', borderRadius: '10px', fontWeight: '700', cursor: 'pointer', fontSize: '0.8rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '5px' }}><X size={13} /> مسح الفلاتر</button>
              </div>
              <div ref={searchRef} style={{ position: 'relative' }}>
                <div style={{ position: 'relative' }}>
                  ...
                  <input type="text" placeholder="ابحث باسم المنتج..." value={productSearch} onChange={e => { setProductSearch(e.target.value); setShowResults(true); }} onFocus={() => searchResults.length > 0 && setShowResults(true)} style={{ ...nomInput, paddingRight: '40px', paddingLeft: searching ? '36px' : '14px', fontSize: '0.95rem' }} />
                </div>
                {showResults && searchResults.length > 0 && (
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 2500, backgroundColor: '#fff', border: '1px solid #e5e5e5', borderRadius: '12px', boxShadow: '0 8px 32px rgba(0,0,0,0.12)', maxHeight: '340px', overflowY: 'auto', marginTop: '6px' }}>
                    {searchResults.map((p: any) => (
                      <div key={p.id} onClick={() => addProduct(p)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 14px', cursor: 'pointer', borderBottom: '1px solid #f5f5f5' }} onMouseEnter={e => (e.currentTarget.style.backgroundColor = '#f0fdf4')} onMouseLeave={e => (e.currentTarget.style.backgroundColor = '#fff')}>
                        ...
                        <div style={{ textAlign: 'left', flexShrink: 0 }}>
                          <div style={{ fontWeight: '900', fontSize: '0.92rem' }}>{(p.sale_price > 0 ? p.sale_price : p.regular_price).toLocaleString()} ج.م</div>
                        </div>
            ...
            <div style={nomCard}>
              ...
              <div ref={customerRef} style={{ position: 'relative', marginBottom: '14px' }}>
                <label style={nomLabel}>ابحث عن عميل موجود</label>
                <div style={{ position: 'relative' }}>
                  <Search size={15} color="#aaa" style={{ position: 'absolute', right: '12px', top: '15px', pointerEvents: 'none' }} />
                  <input type="text" placeholder="اكتب اسم العميل أو رقم هاتفه..." value={selectedUser ? `${selectedUser.full_name} — ${selectedUser.phone_number}` : customerSearch} onChange={e => { setSelectedUser(null); searchCustomers(e.target.value); }} style={{ ...nomInput, paddingRight: '38px' }} />
                ...
                {showCustomerResults && customerResults.length > 0 && !selectedUser && (
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 2500, backgroundColor: '#fff', border: '1px solid #e5e5e5', borderRadius: '10px', boxShadow: '0 8px 24px rgba(0,0,0,0.1)', marginTop: '4px' }}>
                    {customerResults.map((u: any) => (
                      <div key={u.id} onClick={() => { setSelectedUser(u); setManualName(u.full_name); setManualPhone(u.phone_number); setShowCustomerResults(false); }} style={{ padding: '10px 14px', cursor: 'pointer', borderBottom: '1px solid #f5f5f5', display: 'flex', alignItems: 'center', gap: '10px' }} onMouseEnter={e => (e.currentTarget.style.backgroundColor = '#f0fdf4')} onMouseLeave={e => (e.currentTarget.style.backgroundColor = '#fff')}>
                        <div style={{ width: '34px', height: '34px', borderRadius: '50%', backgroundColor: '#dcfce7', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><User size={15} color="#16a34a" /></div>
                        <div><div style={{ fontWeight: '800', fontSize: '0.88rem' }}>{u.full_name}</div><div style={{ fontSize: '0.75rem', color: '#888' }}>{u.phone_number}</div></div>
                      </div>
                    ))}
                  </div>
                )}
          ...
          <div style={{ position: 'sticky', top: '10px' }}>
            <div style={{ ...nomCard, border: '2px solid #e8e8e8' }}>
              ...
              {orderItems.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '28px 0', color: '#ccc' }}><ShoppingCart size={44} style={{ margin: '0 auto 10px', display: 'block', opacity: 0.3 }} /><p style={{ fontSize: '0.85rem' }}>لم تُضف منتجات بعد</p></div>
              ) : (
                <>
                  {orderItems.map((item: any) => (
                  ...
                  <div style={{ borderTop: '1px solid #f0f0f0', marginTop: '10px', paddingTop: '10px', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.8rem' }}>
                      ...
                      <input type="number" min={0} value={shippingCost} onChange={e => setShippingCost(Number(e.target.value))} style={{ width: '80px', height: '32px', border: '1px solid #e5e5e5', borderRadius: '8px', textAlign: 'center', fontSize: '0.85rem', outline: 'none' }} />
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: '0.8rem' }}>
                      <label style={{ color: '#666' }}>خصم (ج.م)</label>
                      <input type="number" min={0} value={discountVal} onChange={e => setDiscountVal(Number(e.target.value))} style={{ width: '80px', height: '32px', border: '1px solid #e5e5e5', borderRadius: '8px', textAlign: 'center', fontSize: '0.85rem', outline: 'none' }} />
                    </div>
              ...
              <button onClick={submitOrder} disabled={submitting || orderItems.length === 0} style={{ width: '100%', marginTop: '16px', padding: '13px', backgroundColor: orderItems.length === 0 ? '#ccc' : '#22c55e', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: '900', fontSize: '0.95rem', cursor: orderItems.length === 0 ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}>
...
export default function AdminOrders() {
  ...
  const paymentLabels: any = {
    'card_installments': 'بطاقة / تقسيط', 'instapay': 'انستا باي',
    'wallets': 'محفظة إلكترونية', 'cash': 'كاش عند الاستلام',
    'vodafone_cash': 'فودافون كاش', 'bank_transfer': 'تحويل بنكي',
  };
  ...
  async function fetchOrders() {
    ...
    } catch (err: any) { toast.error('خطأ في جلب الطلبات: ' + err.message); }
  ...
  // ── NEW: Update payment status directly in Supabase ───────────────────────
  async function updatePaymentStatus(orderId: string, newPaymentStatus: string) {
    ...
    try {
      ...
      // Update local state so it reflects immediately everywhere
  ...
  async function saveOrderEdits() {
    try {
      ...
      const updatePayload: any = { items: editedItems, customer_address: editedAddress, city: editedCity, total_price: newTotal.toFixed(2), discount_amount: discAmount, extra_fee: extraFee.amount, extra_fee_reason: extraFee.reason, shipping_removed: removeShipping };
        ...
        <div style={{ padding: '30px 44px' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '28px' }}>
            <div style={{ backgroundColor: '#f9fafb', borderRadius: '14px', padding: '20px', border: '1px solid #f0f0f0' }}>
              ...
              <div style={{ fontSize: '0.82rem', color: '#555', fontWeight: '600', direction: 'ltr' }}>{order.customer_phone}</div>
            </div>
            <div style={{ backgroundColor: '#f9fafb', borderRadius: '14px', padding: '20px', border: '1px solid #f0f0f0' }}>
    ...
    <div style={{ padding: '30px', direction: 'rtl', maxWidth: '1400px', margin: '0 auto', background: '#f8f9fa', minHeight: '100vh' }}>
      ...
      <div style={headerSection}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button onClick={() => setShowNewOrderModal(true)} style={{ display: 'flex', alignItems: 'center', gap: '10px', background: 'linear-gradient(135deg, #22c55e, #16a34a)', color: '#fff', border: 'none', borderRadius: '14px', padding: '13px 24px', fontWeight: '900', fontSize: '1rem', cursor: 'pointer', boxShadow: '0 4px 16px rgba(34,197,94,0.35)' }}>
        ...
        <table style={table}>
          ...
          <tbody>
            {orders.map((order) => {
              ...
              const psColors = paymentStatusColors[ps] || paymentStatusColors.pending;
              return (
                <tr key={order.id} style={tr}>
                  ...
                  <td style={td}>
                    <span style={{ fontSize: '0.75rem', fontWeight: '800', padding: '4px 10px', borderRadius: '8px', background: psColors.bg, color: psColors.color, border: `1px solid ${psColors.border}`, whiteSpace: 'nowrap' }}>
        ...
        <div style={modalOverlay}>
          <div style={{ ...modalContent, maxWidth: '820px' }}>
            <div style={{ display: 'flex', gap: '10px', marginBottom: '20px', alignItems: 'center' }}>
              <div style={{ flex: 1 }}><h2 style={{ margin: 0, fontSize: '1.3rem', fontWeight: '900', color: '#1a1a1a', display: 'flex', alignItems: 'center', gap: '8px' }}><FileText size={22} color="#22c55e" /> ORDER #{selectedOrder.id.slice(0, 8).toUpperCase()}</h2></div>
              <button onClick={handlePrintInvoice} style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '10px 18px', background: '#fff', color: '#1a1a1a', border: '1.5px solid #ddd', borderRadius: '12px', fontWeight: '700', fontSize: '0.88rem', cursor: 'pointer' }}><Printer size={16} /> طباعة</button>
              <button onClick={() => handleDownloadInvoice(selectedOrder)} disabled={isDownloadingPdf} style={{ display: 'flex', alignItems: 'center', gap: '7px', padding: '10px 20px', background: isDownloadingPdf ? '#ccc' : 'linear-gradient(135deg, #22c55e, #16a34a)', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: '800', fontSize: '0.88rem', cursor: isDownloadingPdf ? 'not-allowed' : 'pointer' }}><Download size={16} /> {isDownloadingPdf ? 'جاري التحميل...' : 'تحميل PDF'}</button>
          ...
          <div style={{ ...modalContent, maxWidth: editMode ? '900px' : '750px' }}>
            <div style={modalHeader}>
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                {!editMode && <button onClick={() => setShowInvoice(true)} style={{ display: 'flex', alignItems: 'center', gap: '6px', background: 'linear-gradient(135deg, #0f172a, #1e293b)', color: '#fff', border: 'none', borderRadius: '10px', padding: '9px 16px', cursor: 'pointer', fontWeight: '700', fontSize: '0.88rem' }}><FileText size={16} color="#22c55e" /> عرض ORDER</button>}
                {!editMode ? <button onClick={() => openEditMode(selectedOrder)} style={editBtnStyle}><Edit2 size={16} /> تعديل</button> : <><button onClick={saveOrderEdits} style={saveBtnStyle}><Save size={16} /> حفظ التعديلات</button><button onClick={() => setEditMode(false)} style={cancelBtnStyle}><X size={16} /> إلغاء</button></>}
                <button onClick={() => { setSelectedOrder(null); setEditMode(false); setShowInvoice(false); }} style={closeBtn}><X size={24} /></button>
              </div>
            </div>

            <div style={modalBody}>
              {/* ── Payment Status Card — always visible in detail modal ── */}
                    ...
                    {Object.entries(paymentStatusLabels).map(([key, label]) => {
                      ...
                      return (
                        <button
                          ...
                          style={{
                            padding: '7px 14px', borderRadius: '10px', fontSize: '0.8rem', fontWeight: '800',
                            cursor: isActive || updatingPayment ? 'not-allowed' : 'pointer',
                          ...
                          onMouseEnter={e => { if (!isActive) { (e.currentTarget as HTMLButtonElement).style.background = c.bg; (e.currentTarget as HTMLButtonElement).style.color = c.color; (e.currentTarget as HTMLButtonElement).style.border = `1.5px solid ${c.border}`; (e.currentTarget as HTMLButtonElement).style.opacity = '1'; } }}
                          onMouseLeave={e => { if (!isActive) { (e.currentTarget as HTMLButtonElement).style.background = '#fff'; (e.currentTarget as HTMLButtonElement).style.color = '#888'; (e.currentTarget as HTMLButtonElement).style.border = '1.5px solid #e5e7eb'; (e.currentTarget as HTMLButtonElement).style.opacity = '0.75'; } }}
                        >
                ...
                {!editMode ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '12px' }}>
                      <div style={{ flex: 1 }}><label style={labelStyle}>المحافظة</label><input value={editedCity} onChange={e => setEditedCity(e.target.value)} style={inputStyle} /></div>
                      <div style={{ flex: 2 }}><label style={labelStyle}>العنوان</label><input value={editedAddress} onChange={e => setEditedAddress(e.target.value)} style={inputStyle} /></div>
                    </div>
                      ...
                      <div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', marginTop: '6px' }}>
                          {customerAddresses.map((addr: any, i: number) => (
                            <button key={i} onClick={() => { setEditedAddress(addr.address); setEditedCity(addr.city); }} style={{ ...addrBtnStyle, border: editedAddress === addr.address ? '2px solid #27ae60' : '1px solid #eee' }}>
              ...
              <div style={modalCard}>
                ...
                <div style={itemsContainer}>
                  ...
                  {editMode && (
                    <div>
                      <button onClick={() => setShowAddItem(!showAddItem)} style={addItemBtnStyle}><Plus size={16} /> {showAddItem ? 'إخفاء البحث' : 'إضافة منتج'}</button>
                      {showAddItem && (
                        <div style={{ background: '#f0fdf4', borderRadius: '16px', padding: '20px', marginTop: '12px', border: '1px solid #dcfce7' }}>
                          <div style={{ marginBottom: '16px' }}>
                            ...
                            <div style={{ display: 'flex', gap: '8px' }}>
                              <input value={productSearchQuery} onChange={e => setProductSearchQuery(e.target.value)} onKeyDown={e => e.key === 'Enter' && fetchFilteredProducts()} placeholder="اكتب اسم المنتج..." style={{ ...inputStyle, flex: 1 }} />
                              ...
                              <div key={prod.id} style={searchProductRow}>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                  ...
                                  <div>
                                    ...
                                    <div style={{ fontSize: '0.75rem', color: '#888' }}>{prod.brand}</div>
                                  </div>
                                </div>
                                <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                    ...
                    <div style={{ background: '#fffbeb', borderRadius: '16px', padding: '20px', border: '1px solid #fef3c7', display: 'flex', flexDirection: 'column', gap: '16px' }}>
                      ...
                      <div>
                        ...
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                          ...
                          <input type="number" min={0} value={discount.value || ''} onChange={e => setDiscount(d => ({ ...d, value: parseFloat(e.target.value) || 0 }))} style={{ ...inputStyle, width: '120px' }} />
                        </div>
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
              ...
              <div style={modalCard}>
                <h3 style={cardTitle}><CreditCard size={18} /> إثبات الدفع</h3>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '15px' }}>
                  <span>الوسيلة: <strong>{paymentLabels[selectedOrder.payment_method] || selectedOrder.payment_method}</strong></span>
                  {selectedOrder.payment_screenshot_url && <a href={selectedOrder.payment_screenshot_url} target="_blank" rel="noreferrer" style={viewLink}><ExternalLink size={16} /> فتح الصورة الأصلية</a>}
                </div>
                {selectedOrder.payment_screenshot_url && <div style={imagePreviewBox}><img src={selectedOrder.payment_screenshot_url} alt="Proof" style={fullImg} /></div>}
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.6)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, backdropFilter: 'blur(4px)' }}>
          <div style={{ backgroundColor: '#fff', borderRadius: '20px', padding: '40px 36px', maxWidth: '400px', width: '90%', textAlign: 'center', boxShadow: '0 20px 60px rgba(0,0,0,0.25)' }}>
            ...
            <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
              <button onClick={() => handleDelete(deleteConfirmId)} style={{ padding: '12px 32px', backgroundColor: '#dc2626', color: '#fff', border: 'none', borderRadius: '12px', fontWeight: '800', fontSize: '1rem', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '8px' }}><Trash2 size={16} /> نعم، احذف</button>
              <button onClick={() => setDeleteConfirmId(null)} style={{ padding: '12px 32px', backgroundColor: '#f5f5f5', color: '#555', border: 'none', borderRadius: '12px', fontWeight: '700', fontSize: '1rem', cursor: 'pointer' }}>إلغاء</button>
            </div>
          </div>
        </div>
      )}
...
const miniSelect = (s: string): any => ({ background: s === 'pending' ? '#fff7ed' : s === 'delivered' ? '#f0fdf4' : s === 'shipped' ? '#eff6ff' : '#fef3c7', color: s === 'pending' ? '#c2410c' : s === 'delivered' ? '#15803d' : s === 'shipped' ? '#1e40af' : '#ca8a04', border: `1px solid ${s === 'pending' ? '#ffedd5' : s === 'delivered' ? '#dcfce7' : s === 'shipped' ? '#dbeafe' : '#fef3c7'}`, padding: '6px 10px', borderRadius: '8px', cursor: 'pointer', outline: 'none', fontSize: '0.8rem', fontWeight: 'bold' });
