...
import { 
  ...
  CarFront, Globe, Factory, Calendar, Smartphone
} from 'lucide-react';
          ...
          <tbody>
            {orders.map((order) => (
              <tr key={order.id} style={tr}>
                ...
                <td style={td}>
                  {/* تم حذف خاصية title تماماً هنا لتجنب أخطاء TypeScript */}
                    ...
                    <div key={i} style={productDetailCard}>
                      <div style={{ display: 'flex', gap: '15px', alignItems: 'center' }}>
                        <div style={miniProductImgBox}>
                          <img src={item.image || 'https://via.placeholder.com/150'} alt="" style={miniProductImg} />
                        </div>
                        <div style={{ flex: 1 }}>
...
const miniSelect = (s:string): any => ({ background: s === 'pending' ? '#f39c1220' : '#000', color: s === 'pending' ? '#f39c12' : '#2ecc71', border: '1px solid #1a1a1a', padding: '4px 8px', borderRadius: '6px', cursor: 'pointer', outline: 'none', fontSize: '0.75rem' });
