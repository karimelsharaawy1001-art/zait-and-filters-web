...
import { Eye, Edit3, DollarSign, Trash2, Check, X, FileDown, FileUp, ClipboardList, CheckSquare, Square, Minus, AlertTriangle } from 'lucide-react';
...
function ConfirmModal({
  ...
  return (
    <div style={{
      ...
      <div style={{
        backgroundColor: '#111', border: '1px solid #ff4d4d44', borderRadius: '16px',
        padding: '32px', maxWidth: '420px', width: '90%', textAlign: 'center',
        boxShadow: '0 20px 60px rgba(0,0,0,0.5)',
      }}>
        <AlertTriangle size={48} color="#ff4d4d" style={{ margin: '0 auto 16px' }} />
        <p style={{ color: '#fff', fontSize: '1rem', fontWeight: '700', lineHeight: '1.6', marginBottom: '28px' }}>
          {message}
        </p>
        <div style={{ display: 'flex', gap: '12px', justifyContent: 'center' }}>
          <button
            onClick={onCancel}
            style={{ padding: '12px 28px', backgroundColor: '#1a1a1a', color: '#aaa', border: '1px solid #333', borderRadius: '10px', cursor: 'pointer', fontWeight: '700', fontSize: '0.9rem' }}
          >
            إلغاء
          </button>
          <button
            onClick={onConfirm}
            style={{ padding: '12px 28px', backgroundColor: '#ff4d4d', color: '#fff', border: 'none', borderRadius: '10px', cursor: 'pointer', fontWeight: '900', fontSize: '0.9rem' }}
          >
            تأكيد الحذف
          </button>
        </div>
...
export default function AdminProducts() {
  ...
  const handleImport = async (e: any) => {
    ...
    reader.onload = async (event: any) => {
      ...
      setImporting(true);
      const importToast = toast.loading(`جاري استيراد ${products.length} منتج...`);
      ...
      try {
        ...
        if (result.error) {
          ...
          toast.success(`✅ تحديث ${result.updateCount} | إضافة ${result.insertCount}`, { duration: 5000 });
    ...
    <div style={{ direction: 'rtl', color: '#fff', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
        ...
        <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
          ...
          <label style={{ ...secondaryBtnStyle, opacity: importing ? 0.6 : 1, pointerEvents: importing ? 'none' : 'auto' }}>
      ...
      {selectedIds.size > 0 && (
        <div style={bulkBarStyle}>
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            <button onClick={() => setSelectedIds(new Set())} style={{ ...secondaryBtnStyle, color: '#aaa' }}>
              <X size={15} /> إلغاء التحديد
            </button>
            <button onClick={handleBulkDelete} disabled={bulkDeleting} style={{ ...secondaryBtnStyle, backgroundColor: '#ff4d4d', color: '#fff', border: 'none' }}>
              <Trash2 size={15} />
              ...
              return (
                ...
                >
                  <td style={{ ...tdStyle, textAlign: 'center', width: '40px' }}>
                    <button onClick={() => toggleSelectOne(product.id)} style={{ background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto' }}>
                      {isSelected ? <CheckSquare size={18} color="#ff4d4d" /> : <Square size={18} color="#444" />}
                    </button>
                  </td>
                  <td style={tdStyle}>
